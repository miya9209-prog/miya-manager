...
def render_dashboard(db_df: pd.DataFrame, log_df: pd.DataFrame):
    ...
    with c1:
        st.metric("현재 DB 상품 수", f"{len(db_df):,}" if db_df is not None else "0")
    with c2:
        missing_size = 0 if db_df is None or db_df.empty else int((db_df["size_range"] == "").sum())
        st.metric("사이즈 범위 누락", f"{missing_size:,}")
    with c3:
        missing_color = 0 if db_df is None or db_df.empty else int((db_df["color_options"] == "").sum())
        st.metric("컬러 정보 누락", f"{missing_color:,}")
    with c4:
        missing_summary = 0 if db_df is None or db_df.empty else int((db_df["product_summary"] == "").sum())
        st.metric("요약 누락", f"{missing_summary:,}")

    if db_df is not None and not db_df.empty:
        st.markdown("##### DB 품질 점검")
        issues = pd.DataFrame({
            "점검항목": ["사이즈 범위 누락", "컬러 누락", "카테고리 누락", "요약 누락", "실측정보 없음"],
            "건수": [
                int((db_df["size_range"] == "").sum()),
                int((db_df["color_options"] == "").sum()),
                int((db_df["category"] == "").sum()),
                int((db_df["product_summary"] == "").sum()),
                int((db_df["measurement_source"] == "").sum()),
            ]
        })
        st.dataframe(issues, use_container_width=True, hide_index=True)

    if log_df is not None and not log_df.empty:
        st.markdown("##### 미야언니 V2 통계 요약")
        lc1, lc2, lc3, lc4 = st.columns(4)
        with lc1:
            st.metric("총 로그 수", f"{len(log_df):,}")
        with lc2:
            st.metric("임시답변 전환", f"{int(log_df['is_fallback'].sum()):,}")
        with lc3:
            st.metric("에러 수", f"{int(log_df['is_error'].sum()):,}")
        with lc4:
            avg_latency = pd.to_numeric(log_df["latency_ms"], errors="coerce").dropna().mean()
            st.metric("평균 응답(ms)", f"{avg_latency:,.0f}" if pd.notna(avg_latency) else "-")

        by_date = log_df.dropna(subset=["timestamp"]).copy()
        if not by_date.empty:
            daily = by_date.groupby(by_date["timestamp"].dt.date).size().reset_index(name="count").set_index("timestamp")
            st.line_chart(daily)


def render_db_generation():
    st.subheader("자동 상품 DB 생성")
    st.caption("형준님 기존 DB 생성기와 같은 컬럼 구조와 같은 파일명(misharp_miya_db.csv)으로 출력됩니다.")

    with st.sidebar:
        st.markdown("### DB 생성 설정")
    ...
    with col2:
        run = st.button("DB 생성 시작", type="primary", use_container_width=True)
    ...
    if run and input_text.strip():
        ...
        st.download_button("misharp_miya_db.csv 다운로드", data=csv_bytes, file_name="misharp_miya_db.csv", mime="text/csv", use_container_width=True)
...
def render_db_compare(current_db_df: pd.DataFrame):
    st.subheader("반자동 DB 업로드 준비")
    st.caption("GitHub 직접 업로드 대신, 현재 DB와 신규 DB를 비교해서 전체 교체용 파일과 신규 상품용 파일을 내려받는 화면입니다.")
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.metric("현재 DB 상품 수", f"{len(current_db_df):,}")
    with c2:
        st.metric("신규 DB 상품 수", f"{len(new_db_df):,}")
    with c3:
        st.metric("신규 추가 상품", f"{len(added_df):,}")
    with c4:
        st.metric("변경 상품", f"{len(changed_df):,}")
    ...
    with dl1:
        st.download_button(
            "전체 교체용 misharp_miya_db.csv 다운로드",
            data=new_db_df.to_csv(index=False).encode("utf-8-sig"),
    ...
    with dl2:
        st.download_button(
            "신규 상품만 별도 다운로드",
            data=added_df.to_csv(index=False).encode("utf-8-sig"),
            file_name="misharp_miya_db_new_only.csv",
            mime="text/csv",
            use_container_width=True,
        )
    ...
    st.markdown(
        ...
        "4. 혹시 불안하면 `신규 상품만 파일`로 먼저 점검 후 전체 교체하세요."
    )


def render_stats(log_df: pd.DataFrame, db_df: pd.DataFrame):
    st.subheader("미야언니 V2 통계 분석")
    st.caption("CSV, XLSX, JSON, JSONL 로그를 올리면 임시답변 전환, 에러, 상품별 상담량 등을 분석합니다.")

    if log_df is None or log_df.empty:
        st.info("로그 파일이 없어서 통계를 계산할 수 없습니다. 아래 템플릿을 내려받아 로그 포맷을 맞추면 바로 분석됩니다.")
    ...
    avg_latency = pd.to_numeric(log_df["latency_ms"], errors="coerce").dropna().mean()
    with m1:
        st.metric("총 로그", f"{total_logs:,}")
    with m2:
        st.metric("임시답변 전환", f"{fallback_cnt:,}", f"{(fallback_cnt / total_logs * 100):.1f}%" if total_logs else None)
    with m3:
        st.metric("에러", f"{error_cnt:,}", f"{(error_cnt / total_logs * 100):.1f}%" if total_logs else None)
    with m4:
        st.metric("RateLimit", f"{rate_cnt:,}")
    with m5:
        st.metric("평균 응답(ms)", f"{avg_latency:,.0f}" if pd.notna(avg_latency) else "-")

    if log_df["timestamp"].notna().any():
        st.markdown("##### 일별 로그 추이")
        daily = log_df.dropna(subset=["timestamp"]).groupby(log_df.dropna(subset=["timestamp"])["timestamp"].dt.date).agg(
    ...
    insights = []
    if total_logs:
        if fallback_cnt / total_logs >= 0.1:
            insights.append("임시답변 전환 비율이 10% 이상입니다. DB 누락 상품 또는 프롬프트 가드레일 충돌 여부를 점검하는 게 좋습니다.")
        if error_cnt / total_logs >= 0.03:
            insights.append("에러 비율이 3% 이상입니다. OpenAI 호출 실패, timeout, 응답 재시도 정책을 우선 확인해보세요.")
    if not grouped.empty and grouped.iloc[0]["임시답변전환"] >= 3:
        insights.append(f"`{grouped.iloc[0]['product_name'] or grouped.iloc[0]['product_no']}` 상품은 임시답변 전환이 많아 DB 보강 우선순위로 보입니다.")
    if not insights:
        insights.append("현재 로그 기준으로는 큰 이상 징후가 두드러지지 않습니다. 다음으로는 상품별 클릭수/상담수 연결 로그를 추가하면 더 실무형 분석이 가능합니다.")
...
def main():
    ...
    st.caption("자동 상품DB 생성 · 반자동 DB 업로드 준비 · 미야언니 V2 통계 분석")

    with st.sidebar:
        page = st.radio(
            "메뉴",
            ["대시보드", "자동 상품DB 생성", "반자동 DB 업로드 준비", "미야언니 V2 통계 분석"],
            index=0,
        )
        st.markdown("---")
        current_db_file = st.file_uploader("현재 미야언니 DB 업로드", type=["csv", "xlsx"], key="current_db")
        log_file = st.file_uploader("미야언니 V2 로그 업로드", type=["csv", "xlsx", "json", "jsonl"], key="log_file")

    current_db_df = ensure_db_columns(parse_uploaded_table(current_db_file)) if current_db_file else pd.DataFrame(columns=DB_COLUMNS)
    raw_log_df = parse_uploaded_table(log_file) if log_file else pd.DataFrame()
    ...
    elif page == "자동 상품DB 생성":
        render_db_generation()
    elif page == "반자동 DB 업로드 준비":
        render_db_compare(current_db_df)
    elif page == "미야언니 V2 통계 분석":
